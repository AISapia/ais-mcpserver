...
class RulesMCPServer {
    ...
    calculateSimilarity(query, rule) {
        ...
        const tagMatches = rule.tags.filter(tag => tag.toLowerCase().includes(queryLower) || queryLower.includes(tag.toLowerCase())).length;
        score += tagMatches * 40;
        ...
        const exampleMatches = rule.examples.filter(example => example.toLowerCase().includes(queryLower)).length;
...
Tags: ${rule.tags.join(", ")}`,
    ...
    async searchRules(args) {
        const { query, category, tags, limit = 5 } = args;
        let filteredRules = this.rules;
        // Apply filters
        if (category) {
            filteredRules = filteredRules.filter(r => r.category.toLowerCase().includes(category.toLowerCase()));
        }
        if (tags && tags.length > 0) {
            filteredRules = filteredRules.filter(r => tags.some((tag) => r.tags.some(ruleTag => ruleTag.toLowerCase().includes(tag.toLowerCase()))));
        }
        ...
        const resultsText = results.map((result, index) => `${index + 1}. [Score: ${result.similarity}] ${result.rule.title} (${result.rule.id})
...
${relatedRules ? `Related Rules:\n   ${relatedRules}` : ""}`,
    ...
    async listCategories() {
        const categoryList = this.categories.map(category => `${category.name}: ${category.description}
   Rules: ${category.rules.length}
   Rule IDs: ${category.rules.join(", ")}`).join("\n\n");
        return {
            content: [
                {
                    type: "text",
...
${categoryList}`,
    ...
    async getRulesByCategory(args) {
        const { category } = args;
        const rules = this.rules.filter(r => r.category.toLowerCase().includes(category.toLowerCase()));
        if (rules.length === 0) {
            return {
                content: [
                    {
        ...
        const rulesList = rules.map(rule => `${rule.id}: ${rule.title}
   Description: ${rule.description}
   Tags: ${rule.tags.join(", ")}`).join("\n\n");
        return {
            content: [
                {
                    type: "text",
...
${rulesList}`,
    ...
    async exportRules(args) {
        const { format = "json", category } = args;
        let rulesToExport = this.rules;
        if (category) {
            rulesToExport = this.rules.filter(r => r.category.toLowerCase().includes(category.toLowerCase()));
        }
        ...
        if (format === "json") {
            ...
            const markdown = rulesToExport.map(rule => `# ${rule.title} (${rule.id})
