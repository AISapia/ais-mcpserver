...
Timestamp: ${memory.timestamp}`,
    ...
    calculateSimilarity(query, memory) {
        ...
        const tagMatches = memory.tags.filter(tag => tag.toLowerCase().includes(queryLower) || queryLower.includes(tag.toLowerCase())).length;
        score += tagMatches * 30;
    ...
    async searchMemories(args) {
        ...
        // Apply filters
        if (project) {
            filteredMemories = filteredMemories.filter(m => m.project.toLowerCase().includes(project.toLowerCase()));
        }
        if (category) {
            filteredMemories = filteredMemories.filter(m => m.category.toLowerCase().includes(category.toLowerCase()));
        }
        if (tags && tags.length > 0) {
            filteredMemories = filteredMemories.filter(m => tags.some((tag) => m.tags.some(memTag => memTag.toLowerCase().includes(tag.toLowerCase()))));
        }
        ...
        const results = filteredMemories
            ...
            relevantTags: memory.tags.filter(tag => tag.toLowerCase().includes(query.toLowerCase()) ||
        ...
        const resultsText = results.map((result, index) => `${index + 1}. [Score: ${result.similarity}] ${result.entry.title}
...
${this.memories
    ...
    async exportMemories(args) {
        const { format = "json", project } = args;
        let memoriesToExport = this.memories;
        if (project) {
            memoriesToExport = this.memories.filter(m => m.project.toLowerCase().includes(project.toLowerCase()));
        }
        ...
        if (format === "json") {
            ...
            const markdown = memoriesToExport.map(memory => `# ${memory.title}
