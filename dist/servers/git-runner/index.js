...
Interval: ${this.state.commitInterval} minutes`,
    ...
    determineChangeType(diff) {
        ...
        if (diff.includes("function") || diff.includes("class"))
            return "refactor";
        if (diff.includes("bug") || diff.includes("error") || diff.includes("fix"))
            return "fix";
        return "update";
    }
    getFeatureDescription(files, diff) {
        const fileTypes = this.analyzeFileTypes(files);
        const mainType = Object.keys(fileTypes)[0];
        return `new ${mainType} functionality`;
    }
    getFixDescription(files, diff) {
        return "issues in codebase";
    }
    getUpdateDescription(files) {
    ...
    getRefactorDescription(files) {
        const fileTypes = this.analyzeFileTypes(files);
        const mainType = Object.keys(fileTypes)[0];
        return `${mainType} code structure`;
    }
    async gitCommitWithAI(args = {}) {
        try {
